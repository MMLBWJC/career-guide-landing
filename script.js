...
function splitTextNode(text, counter) {
  ...
  Array.from(normalized).forEach((char) => {
    ...
    span.style.setProperty("--char-index", counter.value);
...
function splitHeroTitle() {
  ...
  sourceNodes.forEach((node) => {
    if (node.nodeType === Node.TEXT_NODE) {
      line.append(splitTextNode(node.textContent, counter));
      return;
    }
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      clone.append(splitTextNode(node.textContent, counter));
      line.append(clone);
    }
  });
